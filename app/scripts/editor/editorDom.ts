...
export function buildEditorDom(container: HTMLElement, callbacks: EditorToolbarCallbacks = {}): EditorDomRefs {
  ...
  pathConfirmBox.style.width = '480px';
  ...
  const pathConfirmInputRow = el('div');
  pathConfirmInputRow.style.cssText = 'display:flex;align-items:center;gap:4px;margin-bottom:8px;';
  const pathConfirmPathInput = el('input', { type: 'text', id: EditorIds.pathConfirmPathInput, placeholder: 'folder/path' });
  pathConfirmPathInput.style.flex = '1';
  const pathConfirmSeparator = el('span');
  pathConfirmSeparator.textContent = '/';
  pathConfirmSeparator.style.cssText = 'flex-shrink:0;padding:0 2px;';
  const pathConfirmInput = el('input', { type: 'text', id: EditorIds.pathConfirmInput, placeholder: 'filename.ext' });
  pathConfirmInput.style.flex = '1';
  pathConfirmInputRow.appendChild(pathConfirmPathInput);
  pathConfirmInputRow.appendChild(pathConfirmSeparator);
  pathConfirmInputRow.appendChild(pathConfirmInput);
  ...
  pathConfirmForm.appendChild(pathConfirmInputRow);
